...
def buy():
    ...
    with get_db_session():
        order = order_service.create_order(user.id, product, order_type)

    try:
        pay_url = alipay_service.build_pay_url(config.alipay, product, order, device)
...
def alipay_notify():
    ...
    # 验签
    if not alipay_service.verify_notify(config.alipay, post_data):
    ...
    if trade_status in ('TRADE_SUCCESS', 'TRADE_FINISHED'):
        with get_db_session():
            order_service.confirm_paid(out_trade_no, trade_no)
